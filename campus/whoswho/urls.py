...
from .views import RequestUser, UserDetails, AddPerson, UserHome, SearchUsers, RemovePerson

urlpatterns = [
    url(r'^request-user$', RequestUser.as_view(), name='request-user'),
    url(r'^search$', SearchUsers.as_view(), name='search-user'),
    url(r'^add-godchild(?P<is_gp>True|False)$', AddPerson.as_view(), name='add-person'),
